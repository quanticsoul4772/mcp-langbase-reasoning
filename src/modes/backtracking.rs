...
mod tests {
    ...
    fn create_test_config() -> Config {
        use crate::config::{
            DatabaseConfig, ErrorHandlingConfig, LangbaseConfig, LogFormat, LoggingConfig,
            PipeConfig,
        };
    ...
    fn test_backtracking_response_from_json() {
        ...
        let resp = BacktrackingResponse::from_completion(json).unwrap();
    ...
    #[test]
    fn test_backtracking_response_from_plain_text_returns_error() {
        let text = "Just plain text";
        // Non-JSON input returns error
        let result = BacktrackingResponse::from_completion(text);
    ...
    #[test]
    fn test_backtracking_response_invalid_json_returns_error() {
        let invalid = "{ invalid json }";

        // All parse failures return errors (no fallbacks)
        let result = BacktrackingResponse::from_completion(invalid);
        assert!(result.is_err());
    ...
    #[test]
    fn test_backtracking_response_empty_string_returns_error() {
        let empty = "";

        // Empty string is not valid JSON, returns error
        let result = BacktrackingResponse::from_completion(empty);
        assert!(result.is_err());
    }
    ...
    #[test]
    fn test_backtracking_response_missing_required_fields_returns_error() {
        // Test when required fields are missing - returns error
        let json = r#"{"confidence": 0.9}"#; // Missing "thought"

        let result = BacktrackingResponse::from_completion(json);
        assert!(result.is_err());
    }
    ...
    fn test_backtracking_response_clone() {
        ...
        let resp1 = BacktrackingResponse::from_completion(json).unwrap();
